...
function polymorphic() {
		...
		candidate = candidate.filter(function(item) {
			var variadic = false,
				min = item.arguments.map(function(argument, index) {
					...
					return +(!(variadic || 'value'  in argument));
				}).join('').match(/^1+/);
	...
	function matchSignature(list, arg) {
		...
		return list.filter(function(config) {
			...
			//  result is true if no more arguments are provided than the signature allows OR the last argument in the signature is variadic
			result = arg.length <= config.arguments.length || config.arguments[config.arguments.length - 1].type === '...';

			//  test each given argument agains the configured signatures
			if (result) {
				arg.forEach(function(value, index) {
					//  look at ourself and ahead - if there is a following item, and it is variadic, it may be left out entirely (zero or more)
					if (isTypeAtIndex('...', config.arguments, index) || isTypeAtIndex('...', config.arguments, index + 1)) {
						variadic = true;
					}

					//  the result remains valid as long as the values match the given signature (type matches or it is variadic)
					result = result && (variadic || types[index].test(config.arguments[index].type));
				});
			}
		...
		return list.map(function(config) {
			var item = {
					...
					specificity: config.arguments.map(function(argument, index) {
						...
						//  if the argument is a variadic one, we do not count it for the specificity
						if (argument.type === '...') {
							return 0;
						}
					...
					param: config.arguments.map(function(argument, index) {
						//  if a variadic type is encountered, two things are known:
						//    1) there will not be another variable to check
						//    2) there should be no type checking in here
						//  hence we simply use the remainder of the given arguments
						if (argument.type === '...') {
							return arg.slice(index);
						}

						return index < arg.length && typeof arg[index] !== 'undefined' && arg[index] !== null ? arg[index] : argument.value;
					})
				};
	...
	function result() {
		...
		return candidate.call.apply(null, candidate.param);
	}
